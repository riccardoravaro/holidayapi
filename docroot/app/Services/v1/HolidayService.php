   ...
   public function __construct($cache = false)
    ...
    public function getHolidays($parameters)
    {
        ...
        try {
            ...
            } elseif (!isset($parameters['year']) || trim($parameters['year']) == '') {
            ...
            $bank_holiday = isset($parameters['bank_holiday']);
            $date     = $year . '-' . $month . '-' . $day;
            ...
            $country_holidays = $this->calculateHolidays($country, $year, $previous || $upcoming, $bank_holiday);
        } catch (\Exception $e) {
            $payload['status'] = 400;
    ...
    private function calculateHolidays($country, $year, $range = false, $bank_holiday = false)
    {
        $return = [];
        ...
        foreach ($years as $year) {
            ...
            if ($country_holidays) {
                ...
                $holidays = Holidays::where(array('country' => $country ))->get();
              } else {
              	$holidays = Holidays::where(array('country' => $country, 'bank_holiday' => 1 ))->get();
              }
                if (empty($holidays)) {
